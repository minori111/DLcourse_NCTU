...
def main():
    ...
    initial_cut = 0
    name = []
    for i, cut in enumerate(int(np.linspace(0, len(train_dataset), 51))):
        save_pickle(train_dataset[initial_cut:cut], 'data/train/train%d.annotations.pkl' % (i))
        initial_cut = cut
        name.append('train%d' % i)
    ...
    save_pickle(word_to_idx, './data/%s/word_to_idx.pkl' % 'train')
#    captions = _build_caption_vector(annotations=train_dataset, word_to_idx=word_to_idx, max_length=max_length)
#    save_pickle(captions, './data/%s/%s.captions.pkl' % ('train', 'train'))
    file_names, id_to_idx = _build_file_names(train_dataset)
    save_pickle(file_names, './data/%s/%s.file.names.pkl' % ('train', 'train'))
    image_idxs = _build_image_idxs(train_dataset, id_to_idx)
    save_pickle(image_idxs, './data/%s/%s.image.idxs.pkl' % ('train', 'train'))
    # prepare reference captions to compute bleu scores later
    image_ids = {}
    feature_to_captions = {}
    i = -1
    for caption, image_id in zip(train_dataset['caption'], train_dataset['image_id']):
        if not image_id in image_ids:
            image_ids[image_id] = 0
            i += 1
            feature_to_captions[i] = []
        feature_to_captions[i].append(caption.lower() + ' .')
    save_pickle(feature_to_captions, './data/%s/%s.references.pkl' % ('train', 'train'))
    print("Finished building %s caption dataset" %'train')
