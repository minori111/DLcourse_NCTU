...
def load_coco_data(data_path='./data', split='train'):
    ...
    data['features'] = pickle.load(os.path.join(data_path, '%s/%s.features.pkl' % (split, split)))
    if 'train' in split:
        split = 'train'
    with open(os.path.join(data_path, '%s/%s.file.names.pkl' % (split, split)), 'rb') as f:
        data['file_names'] = pickle.load(f)   
    with open(os.path.join(data_path, '%s/%s.captions.pkl' % (split, split)), 'rb') as f:
        data['captions'] = pickle.load(f)
    with open(os.path.join(data_path, '%s/%s.image.idxs.pkl' % (split, split)), 'rb') as f:
        data['image_idxs'] = pickle.load(f)
             
    if split == 'train':       
        with open(os.path.join(data_path, '%s/word_to_idx.pkl' %split), 'rb') as f:
